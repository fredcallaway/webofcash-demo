...
*/
var FOO, SCORE, TIME_LEFT, WATCH, mdp;
...
jsPsych.plugins['mouselab-mdp'] = (function() {
  var Arrow, DEMO_MODEL, Edge, KEYS, LOG_DEBUG, LOG_INFO, MouselabMDP, NEUTRAL_COLOR, NULL, PRINT, RIGHT_MESSAGE, SIZE, STATE_COLOR, State, TOP_ADJUST, TRIAL_INDEX, Text, angle, checkObj, dist, plugin, polarMove, redGreen, remove, removePrivate, round, waitKey;
  PRINT = function(...args) {
  ...
  MouselabMDP = class MouselabMDP {
    constructor(config) {
      ...
      //   @canvas.renderAll()
      this.comprehensionCheck = this.comprehensionCheck.bind(this);
      this.comprehensionAgain = this.comprehensionAgain.bind(this);
      this.comprehensionFailed = this.comprehensionFailed.bind(this);
      this.showExample = this.showExample.bind(this);
      this.clearExample = this.clearExample.bind(this);
      this.showPredictions = this.showPredictions.bind(this);
      this.runDemo = this.runDemo.bind(this);
      this.renderFrontier = this.renderFrontier.bind(this);
      this.hideFrontier = this.hideFrontier.bind(this);
      this.expandFrontier = this.expandFrontier.bind(this);
      ...
      this.resetLowerMessage = this.resetLowerMessage.bind(this);
      ...
      this.finishTrial = this.finishTrial.bind(this);
      ...
      if (this.comprehension || this.exampleOnly) {
        this.examplesLeft = -1;
        this.comprehensionButton = $('<button>', {
          text: 'Submit',
          class: 'btn btn-primary btn-lg',
          click: async() => {
            if (this.examplesLeft > 0) {
              this.showExample();
              this.comprehensionButton.prop("disabled", true);
              await sleep(1500);
              return this.comprehensionButton.prop("disabled", false);
            } else if (this.examplesLeft === 0) {
              if (this.exampleOnly) {
                return this.finishTrial();
              } else {
                this.clearExample();
                return this.examplesLeft -= 1;
              }
            } else {
              return this.comprehensionCheck();
            }
          }
        });
        console.log('here I am');
        this.lowerMessage.append(this.comprehensionButton);
      }
    }
    ...
    comprehensionCheck() {
      var attempt;
      attempt = {
        correct: _.isEqual(this.clickedNodes, this.selectNodes),
        clickedNodes: this.clickedNodes.slice()
      };
      this.clickedNodes = Array(_.size(this.graph)).fill(false);
      this.data.comprehension.push(attempt);
      if (attempt.correct) {
        ...
        this.numTries -= 1;
        if (this.numTries <= 0) {
          return this.comprehensionFailed();
        } else {
          return this.comprehensionAgain();
        }
      }
    }
    ...
    comprehensionAgain() {
      ...
      $('<div>', {
        ...
        html: $('<div>', {
          class: 'modal-content',
          html: `<h2>Not quite</h2>\n\n<p>\nTake a look at a few more examples and try again.\n<br>\n<b>You have ${this.numTries} attempts remaining</b>`
        })
      }).appendTo(this.display);
      return $('<button>', {
        text: 'See Examples',
    ...
    showExample() {
      var g, ref, rewards, s;
      if (this.exampleOnly) {
        this.leftMessage.html(`Example: ${this.exampleIdx + 1}/${this.examples.length}`);
      }
      if (this.examplesLeft === 1) {
        this.comprehensionButton.text('Continue');
      } else {
        this.comprehensionButton.text('Next Example');
      }
      rewards = this.examples[this.exampleIdx].stateRewards;
      ref = this.states;
      for (s in ref) {
        g = ref[s];
        if (s !== "0") {
    ...
    clearExample() {
      var g, ref, s;
      this.comprehensionButton.text('Submit');
      ref = this.states;
      for (s in ref) {
        g = ref[s];
        g.setLabel('');
    ...
    showPredictions() {
      var bf, d, diff, hue, j, l, len, len1, len2, light, m, max, n, opt, pred, ref, ref1, ref2, s, sat, v;
      switch (DEMO_MODEL) {
        case 'None':
          ref = _.values(this.states);
          ...
          break;
        case 'Difference':
          opt = remove(this.demo.predictions["Optimal"][this.demo_i], 1);
          bf = remove(this.demo.predictions["BestFirst"][this.demo_i], 1);
          diff = _.zip(opt, bf).map(function([a, b]) {
            return a - b;
          });
          max = _.zip(opt, bf).map(function([a, b]) {
            return Math.max(a, b);
          });
          ref1 = _.zip(max, diff, _.values(this.states));
          ...
          break;
        default:
          pred = remove(this.demo.predictions[DEMO_MODEL][this.demo_i], 1);
          ref2 = _.zip(pred, _.values(this.states));
    ...
    async runDemo() {
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        await sleep(100);
        await waitKey();
        this.clickState(this.states[c], String(c));
        this.canvas.renderAll();
        this.demo_i += 1;
      }
      if (model == null) {
    ...
    renderFrontier() {
      ...
      for (s in _.keys(this.states)) {
        this.states[s].circle.setShadow({
          blur: this.frontier.has(s) ? 30 : 0,
          color: '#FFDD47'
        });
      }
      return this.canvas.renderAll();
    ...
    expandFrontier(s) {
      var action, r, ref, s1;
      if (this.stateDisplay !== 'click') {
        return;
      }
      this.frontier.delete(s);
      if (this.expandOnly) {
        ref = this.graph[s];
        for (action in ref) {
          [r, s1] = ref[action];
          this.frontier.add(s1);
        }
      }
      return this.renderFrontier();
    }
    ...
    move(s0, a, s1) {
      var nClick, newTop, notEnoughClicks, o, r, ref, s, s1g;
      if (this.freeze) {
        LOG_INFO('freeze!');
      ...
      notEnoughClicks = (this.special.startsWith('trainClick')) && nClick < this.minClicks;
      if (notEnoughClicks) {
        this.lowerMessage.html(`<b>For these practice rounds, try inspecting at least ${this.minClicks} nodes before moving</b>`);
    ...
    resetLowerMessage() {
      ...
      return this.resetTimeout = delay(3000, () => {
        this.lowerMessage.html(this.defaultLowerMessage);
        return this.lowerMessage.css('color', 'black');
      });
    }
    ...
    clickState(g, s) {
      ...
      if (this.comprehension) {
        ...
        this.clickedNodes[s] = !this.clickedNodes[s];
        g.setLabel((this.clickedNodes[s] ? 'X' : ''));
      }
      if (this.stateDisplay !== 'click') {
        return;
      }
      ...
      if (this.moved) {
        ...
        this.lowerMessage.css('color', '#FC4754');
        this.resetLowerMessage();
        return;
      }
      if (this.delaying) {
        this.lowerMessage.html("<b>The node inspector is recharging!</b>");
        this.lowerMessage.css('color', '#FC4754');
        this.resetLowerMessage();
        return;
      }
      if (this.expandOnly && !this.frontier.has(s)) {
        this.lowerMessage.html("<b>You can only inspect the highlighted nodes!</b>");
        this.lowerMessage.css('color', '#FC4754');
        this.resetLowerMessage();
        return;
      }
      ...
      if (this.data.queries.click.state.target.length === (this.minClicks - 1)) {
        if (this.special === 'trainClick') {
          this.lowerMessage.html("Continue inspecting nodes or move with the arrow keys.");
        } else if (this.special === 'trainClickBlock') {
          this.lowerMessage.html('<b>Nice job! You can click on more nodes or start moving.</b>');
          this.lowerMessage.css('color', '#000');
        }
      }
      if (this.special === 'trainClickBlock' && this.data.queries.click.state.target.length === (this.minClicks - 1)) {
        this.lowerMessage.html('<b>Nice job! You can click on more nodes or start moving.</b>');
        this.lowerMessage.css('color', '#000');
      }
    ...
    arrive(s, repeat = false) {
      var a, g, keys;
      if (this.exampleOnly) {
        this.examplesLeft = this.examples.length;
        this.showExample();
        return;
      }
      if (this.comprehension) {
    ...
    run() {
      ...
      if (this.expandOnly) {
        ...
        this.frontier = new Set(_.keys(mdp.states));
      }
      if (this.timeLimit || this.minTime) {
        this.startTimer();
      }
      this.expandFrontier(this.initial);
    ...
    buildMap() {
      ...
      for (s in ref) {
        location = ref[s];
        [x, y] = location;
        this.states[String(s)] = new State(s, x - minx, y - miny, {
          fill: '#bbb',
    ...
    endTrial() {
      ...
      return this.keyListener = jsPsych.pluginAPI.getKeyboardResponse({
        ...
        persist: false,
        allow_held_key: false,
        callback_function: this.finishTrial
      });
    }
  ...
  plugin = {
    trial: function(display_element, trialConfig) {
      var trial;
      if (typeof trialConfig.callback === "function") {
